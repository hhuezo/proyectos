...
$sel1=$con->prepare("SELECT ifnull(count(*),0) as cuenta FROM schedule_event s where student_id='$student_id' and date = DATE_ADD(current_date(), INTERVAL 1 DAY)");
$sel1->execute();
...
if ($cuenta > 0) {
    ...
    LEFT OUTER JOIN puntos p ON s.location_id=p.id where student_id='$student_id' and date = DATE_ADD(current_date(), INTERVAL 1 DAY)");
    $sel->execute();
    $res=$sel->get_result();
